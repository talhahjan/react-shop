...
await axios.post(`api/login/google`, user).then((res)=>{
...
let names=response.name.split(' ');
  ...
  let user={
    ...
    await axios.post(`api/login/facebook`, user).then((res)=>{
...
export const SocialGoogleLogin=()=>{
  return (    
    ...
    render={renderProps => (
      <button disabled={renderProps.disabled} className='btn-social btn-outline-instagram btn-social-circle waves-effect waves-light m-1' onClick={renderProps.onClick} disabled={renderProps.disabled}><FaGooglePlusG /></button>
    )}
    /> 
